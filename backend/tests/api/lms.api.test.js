...
async function createUser(username, roleType) {
  ...
  await strapi.db.query('plugin::users-permissions.user').update({
    where: { id: created.id },
    data: { password: await bcrypt.hash('Pass1234!', 10) },
  });
  return created;
}
...
describe('LMS API integration', () => {
  ...
  test('owner creates a lesson and enrollment is idempotent', async () => {
    const lessonResponse = await request(app)
      ...
      .set(auth(instructorToken))
      .send({ title: 'Policies first', content: 'Authorize on the server.', position: 1 })
      .expect(201);
  ...
  test('completion persists and cannot be counted twice', async () => {
    ...
    const second = await request(app).put(path).set(auth(studentToken)).expect(200);
  ...
  test('quiz answers stay private and grading rejects forged scores', async () => {
    const created = await request(app)
      ...
      .send({
        ...
        questions: [
          { prompt: 'Where must access be enforced?', options: ['UI', 'Backend'], correctOption: 1 },
          { prompt: 'Who can enroll?', options: ['Student', 'Instructor'], correctOption: 0 },
        ],
      })
      .expect(201);
    ...
    const studentView = await request(app)
      .get(`/api/lms/my-courses/${course.documentId}/quizzes/${quiz.documentId}`)
      .set(auth(studentToken));
    if (studentView.status !== 200) throw new Error(JSON.stringify(studentView.body));
    expect(studentView.body.data.questions[0]).not.toHaveProperty('correctOption');

    await request(app)
      .post(`/api/lms/my-courses/${course.documentId}/quizzes/${quiz.documentId}/attempts`)
      .set(auth(studentToken))
    ...
    const attempt = await request(app)
      ...
      .send({ answers: [1, 1] })
      .expect(201);
    expect(attempt.body.data).toMatchObject({ score: 1, total: 2, percentage: 50 });
  });
  ...
  test('draft blog is private until its author publishes it', async () => {
    const created = await request(app)
      ...
      .send({ title: 'Policy design', body: 'Keep authorization close to data.' })
      .expect(201);
  ...
  test('only admin can manage roles and view platform stats', async () => {
    ...
    const stats = await request(app)
      ...
      .set(auth(adminToken))
      .expect(200);
    expect(stats.body.data).toMatchObject({ totalCourses: 1, totalEnrollments: 1 });
  });
});
